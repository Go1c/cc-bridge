...
[快速开始](#-快速开始) &bull; [配置说明](#-配置说明) &bull; [API 文档](#-http-api) &bull; [部署指南](./docs/DEPLOYMENT.md)
...
- [快速开始](#-快速开始)
- [配置说明](#-配置说明)
- [构建与部署](#-构建与部署)（完整生产部署见 [docs/DEPLOYMENT.md](./docs/DEPLOYMENT.md)）
- [HTTP API](#-http-api)
...
> **完整项目部署手册**（架构、开发/生产构建、Docker、二进制、systemd、反代、业务配置、防封、升级备份、排障）见：[docs/DEPLOYMENT.md](./docs/DEPLOYMENT.md)
...
```json
{
  ...
  "allowed_system_role_models": ["claude-opus-5", "claude-sonnet-5", "claude-opus-4-8"]
}
```
...
Claude Code 在 `claude-opus-5` / `claude-sonnet-5` / `claude-opus-4-8` 等请求中
可能把运行时提醒放入 `messages[].role=system`。网关通过全局 settings key
`allow_system_role_models` 控制哪些模型允许透传这种格式：

- 默认值：`claude-opus-5,claude-sonnet-5,claude-opus-4-8`
- 格式：逗号分隔的精确模型 ID 列表，例如 `claude-opus-5,claude-sonnet-5,claude-opus-4-8`
